...
def get_data(var,PFT,first_year,last_year,GCM,scen,sens):
    ### Sensitivity sceneriment
    ds_sens = xr.open_dataset(pathwayIN+'/output/netCDF/NHP/runs_'+
                              GCM+'_'+scen+'_'+sens+'/'+var+'_1960-2099.nc')  
    ds_sens = ds_sens.sel(time=slice(first_year,last_year)).mean(dim='time')

    ### 'Control' simulation  
    ds = xr.open_dataset(pathwayIN+'/output/netCDF/NHP/runs_'+
                         GCM+'_'+scen+'/'+var+'_1960-2099.nc')   
    ds = ds.sel(time=slice(first_year,last_year)).mean(dim='time')
...
def make_map(var,PFT,first_year,last_year,scen,sens,position):
    diff=(get_data(var,PFT,first_year,last_year,'CNRM-CERFACS-CNRM-CM5',scen,sens)+ \
          get_data(var,PFT,first_year,last_year,'CSIRO-BOM-ACCESS1-0',scen,sens)+ \
          get_data(var,PFT,first_year,last_year,'MIROC-MIROC5',scen,sens)+ \
          get_data(var,PFT,first_year,last_year,'NOAA-GFDL-GFDL-ESM2M',scen,sens))/4
    ...
    if var == 'cmass':
        cbar = fig.colorbar(p, cax=cax, ticks=levels, orientation='horizontal',
                            extend='neither', 
                            label='$\Delta$ Carbon stored in vegetation [kgC m$^{-2}$]')
    elif var == 'clitter':
        cbar = fig.colorbar(p, cax=cax, ticks=levels, orientation='horizontal',
                            extend='neither', 
                            label='$\Delta$ Carbon stored in litter [kgC m$^{-2}$]')
    elif var == 'fpc':
        cbar = fig.colorbar(p, cax=cax, ticks=levels, orientation='horizontal',
                            extend='neither', 
...
for PFTs, PFTl,p,ti in zip(PFT_short_names, PFT_long_names,positions,title_index):
    make_map(var,PFTs,first_year,last_year,scen,sens,p)
...
plt.savefig('figures/map_'+sens+'_'+var+'_'+scen+'_'+first_year+'-'+
