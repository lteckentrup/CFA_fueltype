...
def grab_data(GCM,scen,sens,TFB_group,var,first_year,last_year):
    ...
    if TFB_group == '':
        ...
        ds_sens = xr.open_dataset(pathwayIN+'/output/netCDF/NHP/runs_'+
                                  GCM+'_'+scen+'_'+sens+'/'+var+'_1960-2099_'+
                                  suffix+'.nc')
    else:
        ...
        ds_sens = xr.open_dataset(pathwayIN+'/output/netCDF/NHP/runs_'+GCM+'_'+
                                  scen+'_'+sens+'/'+TFB_group+'/'+var+'_1960-2099_'+
                                  suffix+'.nc')        

    if sens == 'nofire':   
        diff = ds.sel(time=slice(first_year,last_year))['Total'].values - \
               ds_sens.sel(time=slice(first_year,last_year))['Total'].values
    else:
        diff = ds_sens.sel(time=slice(first_year,last_year))['Total'].values - \
               ds.sel(time=slice(first_year,last_year))['Total'].values       

    return(diff.flatten())

def get_ens_stat(scen,sens,FT_group,var,first_year,last_year):

    ### Create dataframe where each column is the timeseries of one GCM
    df = pd.DataFrame()
    for GCM in GCM_list:
        df[GCM] = grab_data(GCM,scen,sens,FT_group,var,first_year,last_year)

    return(df.sum().mean(),df.sum().std())

def plot_barplot(FT_group,sens,var1,var2,axis):
    ### Set up for loop
    ...
    for scenario in scenarios:
        for start_year, end_year in periods:
            ...
            dict_mean_data[column_name] = [
                get_ens_stat(scenario_fname, sens, FT_group, var, 
...
### Plot barplot
plot_barplot('',sens,'cmass','cmass_wood',ax1)
plot_barplot('East_Gippsland',sens,'cmass','cmass_wood',ax3)
plot_barplot('North_East',sens,'cmass','cmass_wood',ax5)
plot_barplot('Wimmera',sens,'cmass','cmass_wood',ax7)
plot_barplot('Mallee',sens,'cmass','cmass_wood',ax9)

plot_barplot('',sens,'cmass_leaf','clitter',ax2)
plot_barplot('East_Gippsland',sens,'cmass_leaf','clitter',ax4)
plot_barplot('North_East',sens,'cmass_leaf','clitter',ax6)
plot_barplot('Wimmera',sens,'cmass_leaf','clitter',ax8)
plot_barplot('Mallee',sens,'cmass_leaf','clitter',ax10)
...
fig.align_ylabels()
plt.savefig('figures/barplot_'+sens+'_cpool_TFB.pdf')
