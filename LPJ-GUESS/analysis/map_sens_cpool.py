...
def get_data(var,first_year,last_year,GCM,scen,sens):
    ...
    if sens == 'nofire':
        diff = ds['Total'] - ds_sens['Total']
    else:
        diff = ds_sens['Total'] - ds['Total']

    return(diff) 

def make_map(var,first_year,last_year,scen,sens,position):
    diff=(get_data(var,first_year,last_year,'CNRM-CERFACS-CNRM-CM5',scen,sens)+ \
          get_data(var,first_year,last_year,'CSIRO-BOM-ACCESS1-0',scen,sens)+ \
          get_data(var,first_year,last_year,'MIROC-MIROC5',scen,sens)+ \
          get_data(var,first_year,last_year,'NOAA-GFDL-GFDL-ESM2M',scen,sens))/4
...
for vars, vart,p,ti in zip(var_short_names, var_titles,positions,title_index):
    make_map(vars,first_year,last_year,scen,sens,p)
...
plt.savefig('figures/map_'+sens+'_cpool_'+scen+'_'+first_year+'-'+
